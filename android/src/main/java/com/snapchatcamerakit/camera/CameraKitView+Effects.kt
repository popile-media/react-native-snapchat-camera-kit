...
fun CameraKitView.applyLensById(
  lensId: String,
  lensGroups: ReadableArray,
  launchDataMap: ReadableMap?,
  promise: Promise?,
) {
  ...
  this.cameraKitSession!!.lenses.repository.get(LensesComponent.Repository.QueryCriteria.Available(groupIds)) { result ->
    result.whenHasSome { lenses ->
      val lens: LensesComponent.Lens? =
        lenses.find { _lens -> _lens.id == lensId }
...
fun CameraKitView.getLensesByGroupId(
  lensGroups: ReadableArray,
  callback: Callback,
) {
  if (this.cameraKitSession == null) {
