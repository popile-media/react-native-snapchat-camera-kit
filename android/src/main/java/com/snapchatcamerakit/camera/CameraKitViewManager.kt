...
class CameraKitViewManager(reactContext: ReactApplicationContext) : ViewGroupManager<CameraKitView>() {
    ...
    fun setIsActive(view: CameraKitView, isActive: Boolean) {
        view.setActive(isActive)
    }

    @ReactProp(name = "initialCameraFacing")
    fun setInitialCameraFacing(view: CameraKitView, initialCameraFacing: String) {
        view.setInitialCameraFacing(initialCameraFacing)
    }

    @ReactProp(name = "initialLens")
    fun setInitialLens(view: CameraKitView, initialLens: ReadableMap?) {
        if (initialLens == null) {
            return
        }
    ...
    fun setLensGroups(view: CameraKitView, lensGroups: ReadableArray?) {
        if (lensGroups != null) {
    ...
    @ReactProp(name = "zoom")
    fun setZoom(view: CameraKitView, enabled: Boolean) {
        if (enabled) {
            //
        } else {
            //
    ...
    fun setFocus(view: CameraKitView, enabled: Boolean) {
        if (enabled) {
            //
        } else {
            //
    ...
    fun setTorch(view: CameraKitView, options: ReadableMap?) {
        // todo
    }
}
